...
class Test_cloud_installer:
    ...
    def test_clone_cloud(self):
        HEADING()
        cmd = "cd tmp; cloudmesh-installer git clone admin"
        Benchmark.Start()
        result = Shell.run(cmd)
        Benchmark.Stop()
        print(result)
    ...
    def test_install_cloud(self):
        HEADING()
        cmd = "cd tmp; cloudmesh-installer install admin"
        Benchmark.Start()
        result = Shell.run(cmd)
        Benchmark.Stop()
        print(result)
        assert os.path.isdir("tmp/cloudmesh-admin/cloudmesh_admin.egg-info")
